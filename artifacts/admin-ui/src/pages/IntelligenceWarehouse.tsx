...
export default function IntelligenceWarehouse() {
  ...
  useEffect(() => {
    fetch("/admin/intelligence-data/intelligence-warehouse-registry-v0.1.json", {
      ...
      .then((res) => {
        if (!res.ok) {
          throw new Error(`Warehouse registry load failed: ${res.status}`);
        }
      ...
      .then(setRegistry)
      .catch((err) => {
        setError(err?.message || "Warehouse registry load failed");
      });
  }, []);
  ...
  const capabilityRows = useMemo(() => {
    ...
    return registry.capabilities
      .map((capability) => ({
        capability,
        engines:
          registry.summary.engine_capability_counts[capability] ?? 0,
        packages:
          registry.summary.package_capability_counts[capability] ?? 0,
      }))
      .sort(
        (a, b) =>
          b.engines + b.packages - (a.engines + a.packages),
      );
  }, [registry]);
    ...
    return registry.engines
      .filter(
        (engine) =>
          ...
          engine.capability === capabilityFilter,
      )
      .slice(0, 80);
  ...
  const visiblePackages = useMemo(() => {
    ...
    return registry.packages
      .filter(
        (pkg) =>
          capabilityFilter === "all" ||
          pkg.capability === capabilityFilter,
      )
      .slice(0, 80);
  }, [registry, capabilityFilter]);
    ...
    <div className="cms-page">
      <section className="cms-card">
        <p className="cms-kicker">
          Phase H6.1 · Replay Ecosystem Integration
        </p>

        <h1>🧠 Intelligence Warehouse</h1>

        <p>
          Internal visibility layer for ZNBW intelligence engines,
          packages, runtime families, dependencies and hidden product
          surfaces.
        </p>
      </section>
        ...
        <>
          ...
          >
            <section className="cms-card">
              ...
              <p>
                Executable TypeScript scripts discovered under
                api-server/src/scripts.
              </p>
            </section>

            <section className="cms-card">
              <p className="cms-kicker">Intelligence Packages</p>
              <h2>{registry.summary.intelligence_file_count}</h2>
              <p>
                JSON and CSV packages discovered under
                data/intelligence.
              </p>
            </section>

            <section className="cms-card">
              <p className="cms-kicker">Replay Packages</p>
              <h2>{registry.summary.replay_file_count}</h2>
              <p>
                Canonical root-level Replay packages discovered under
                data/replay.
              </p>
            </section>

            <section className="cms-card">
              <p className="cms-kicker">Total Packages</p>
              <h2>{registry.summary.package_count}</h2>
              <p>
                Combined Intelligence and Replay package surface.
              </p>
            </section>
          </div>
          ...
          >
            ...
            <p
              ...
              style={{ marginTop: 12 }}
            >
              {registry.version} · generated{" "}
              {formatDate(registry.generated_at)}
            </p>
          </section>
          ...
          >
            <section className="cms-card">
              ...
              <p className="cms-muted">
                Showing {visibleEngines.length} of{" "}
                ...
                {visibleEngines.map((engine) => (
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ margin: 0 }}
                    >
                      {formatCapability(engine.capability)}
                    </p>
            ...
            <section className="cms-card">
              ...
              <p className="cms-muted">
                Showing {visiblePackages.length} of{" "}
                ...
                {visiblePackages.map((pkg) => (
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ margin: 0 }}
                    >
                      {formatCapability(pkg.capability)}
                      {pkg.package_domain
                        ? ` · ${formatCapability(
                            pkg.package_domain,
                          )}`
          ...
          >
            ...
            <ol>
              {topCapabilities(
                registry.summary.engine_capability_counts,
