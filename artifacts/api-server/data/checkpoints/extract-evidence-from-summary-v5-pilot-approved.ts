...
function getValueLocalContext(text: string, index: number): string {
  ...
  const lastAndBeforeValue = beforeValue.toLowerCase().lastIndexOf(" and ");
  const nextAndAfterValue = afterValue.toLowerCase().indexOf(" and ");

  const localStart = lastAndBeforeValue >= 0 ? lastAndBeforeValue + 5 : 0;
  const localEnd =
    nextAndAfterValue >= 0 ? relativeIndex + nextAndAfterValue : raw.length;
