...
import { getDb, articles, desc, eq } from "@workspace/db";
...
function classifyArticle(article: typeof articles.$inferSelect): Metadata {
  const text = [
    article.title,
    article.excerpt,
    article.category,
...
async function main() {
  ...
  console.log(writeMode ? "WRITE MODE: updating database.\n" : "SAFE MODE: preview only. No database updates.\n");

  for (const article of result) {
    const metadata = classifyArticle(article);
...
main().catch((err) => {
  console.error("backfill_metadata_failed", err);
  process.exit(1);
});
