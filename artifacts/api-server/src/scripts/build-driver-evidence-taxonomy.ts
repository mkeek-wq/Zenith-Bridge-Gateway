...
const outputDir = "data/intelligence-driver-library";
const outputPath = path.join(outputDir, "driver-evidence-taxonomy-v0.1.json");

fs.mkdirSync(outputDir, { recursive: true });

const taxonomy = {
  driver_evidence_taxonomy_version: "driver-evidence-taxonomy-v0.1",
  generated_at: new Date().toISOString(),
  policy: {
    principle:
      "Evidence phrases should map to drivers, attribution buckets, and scoring hints. This taxonomy supports attribution but does not automatically close cases.",
...
console.log({
  driver_evidence_taxonomy_version:
    taxonomy.driver_evidence_taxonomy_version,
  drivers: taxonomy.drivers.length,
  evidence_phrases: taxonomy.drivers.reduce(
    (sum, d) => sum + d.evidence_phrases.length,
    0
  ),
  output: outputPath,
});
