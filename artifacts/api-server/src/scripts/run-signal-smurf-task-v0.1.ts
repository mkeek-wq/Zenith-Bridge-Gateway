...
const task = dispatch.tasks.find((t: any) => t.smurf_id === "SIGNAL");

if (!task) throw new Error("No Signal task found.");
if (task.status !== "ready") throw new Error(`Signal task not ready: ${task.status}`);
...
const output = {
  worker_version: "signal-smurf-task-runner-v0.1",
...
writeJson(path.join(ROOT, "data/intelligence/signal-smurf-task-runner-v0.1.json"), output);
