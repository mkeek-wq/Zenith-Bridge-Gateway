...
const attribution =
  readJsonSafe(path.join(ROOT, "data/intelligence/macro-attribution-engine-v0.1.json")) || {};

const cases: any[] = attribution.case_macro_attributions || [];

const lines: string[] = [];

lines.push("# SMURF Macro Attribution Report v0.1");
lines.push("");
...
for (const c of cases) {
  ...
  if (c.top_macro_drivers?.length) {
    for (const d of c.top_macro_drivers.slice(0, 5)) {
      lines.push(
        `- ${d.macro_key} → ${d.mechanism_id} | score ${d.macro_attribution_score} | channel: ${d.channel}`
      );
    }
  } else {
    lines.push("- None");
...
lines.push("");
lines.push("Macro attribution explains the likely operating environment around each replay case. It does not prove causality, validate mechanisms, or override evidence.");
lines.push("");
lines.push("Cases with higher macro attribution scores may be better candidates for future article-readiness review, especially when mechanism reputation and evidence confidence also improve.");
lines.push("");
...
const output = {
  registry_version: "macro-attribution-report-v0.1",
...
fs.writeFileSync(
  path.join(ROOT, "data/intelligence/macro-attribution-report-v0.1.json"),
  JSON.stringify(output, null, 2)
);

fs.writeFileSync(
  path.join(ROOT, "exports/macro-reports/macro-attribution-report-v0.1.md"),
  markdown
);
...
console.log({
  ...
  output_json: "data/intelligence/macro-attribution-report-v0.1.json",
  output_markdown: "exports/macro-reports/macro-attribution-report-v0.1.md"
});

console.log("\n--- Macro Attribution Report Preview ---\n");
