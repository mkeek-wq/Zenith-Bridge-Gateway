...
const keyTakeaways = brief.key_takeaways
  .map((t: string) => `- ${t}`)
  .join("\n");
...
const output = {
  package_version: "article-draft-generator-v0.2",
...
const jsonPath = path.join(outDir, "article-draft-generator-v0.2.json");
const mdPath = path.join(outDir, "sg-energy-trading-hub-draft-v0.2.md");
