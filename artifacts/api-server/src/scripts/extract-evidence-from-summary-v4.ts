...
const EVIDENCE_VERSION = "evidence-extraction-v0.4";

const projectRoot = process.cwd();
const registryPath = path.join(projectRoot, "data", "document-registry", "latest.json");
const evidenceDir = path.join(projectRoot, "data", "evidence-v4");
...
function getSentenceWindow(text: string, index: number): string {
  ...
  const nextColon = after.indexOf(":");

  const candidates = [nextPeriod, nextSemi, nextColon]
...
function extractEvidence(
  ...
  return matches
    .map((match, index) => {
      ...
      const evidenceWindow = getSentenceWindow(
      summary,
      match.index ?? 0,
      );
