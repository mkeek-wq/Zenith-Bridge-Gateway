...
const opportunityQueue = readJson(
  path.join(root, "data/intelligence/article-opportunity-queue-v0.1.json")
);
...
const opportunityByCandidate = new Map(
  (opportunityQueue.opportunities ?? []).map((opportunity: any) => [
...
const packages = (candidateQueue.candidates ?? []).map((candidate: any) => {
  ...
  const opportunity = opportunityByCandidate.get(candidate.candidate_id) as any;
  ...
  return {
    ...
    readiness_decision: readiness ?? null,
        ...
        intelligence_package: opportunity
      ? {
          ...
          business_implication: opportunity.business_implication ?? null,
          why_it_matters: opportunity.why_it_matters ?? null,
          affected_sectors: opportunity.affected_sectors ?? [],
          source_cases: opportunity.source_cases ?? [],
          evidence_patterns: opportunity.evidence_patterns ?? [],
          editorial_guidance: opportunity.editorial_guidance ?? null,
          governance: opportunity.governance ?? null,
        }
      : null,
...
const registry = {
  workbench_version: "article-workbench-package-v0.2",
  generated_at: new Date().toISOString(),
  purpose:
    "Combines article candidates, intelligence signals, verified datasets, coverage, readiness decisions, graph packages, and source metadata into AI-ready article workbench packages.",
  source_files: {
    ...
    article_opportunity_queue: "data/intelligence/article-opportunity-queue-v0.1.json",
  },
...
const outPath = path.join(root, "data/intelligence/article-workbench-package-v0.2.json");
