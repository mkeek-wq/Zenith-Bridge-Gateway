...
const inPath = path.join(
  ...
  "data",
  "article-generator",
  "article-brief-package-v0.2.json"
);
...
const snapshotPath = path.join(
  process.cwd(),
  "data/article-generator/smurf-intelligence-snapshot-v0.1.json"
);
...
const smurfSnapshotMarkdown = snapshot
...
| Confidence score | ${(snapshot.confidence_score * 100).toFixed(1)}% |
| Historical cases | ${snapshot.historical_cases} |
| Recurrence score | ${(snapshot.recurrence_score * 100).toFixed(1)}% |
| Experience strength | ${(snapshot.experience_strength * 100).toFixed(1)}% |
| Pattern stability | ${snapshot.pattern_stability} |
| Confidence tier | ${snapshot.confidence_tier} |

Most frequent evidence signals:

${snapshot.top_evidence_patterns
  .map((p: any) => `- ${p.phrase} (${p.count} cases)`)
  .join("\n")}
`
  : "";
...
const output = {
  package_version: "article-draft-generator-v0.5",
...
const jsonPath = path.join(outDir, "article-draft-generator-v0.5.json");
const mdPath = path.join(outDir, "sg-energy-trading-hub-draft-v0.5.md");
